...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        foreach (var expense in expenses)
        {
            ...
            //ADD ROW RED WITH TITLE AND MOUNT 
            ...
            //ADD ROW GREEN WITH DATE AND MOUNT VALUE
            row = table.AddRow();
            row.Height = HEIGHT_ROW_EXPENSE_TABLE;
    ...
    private Document CreateDocument(DateOnly month)
    {
        ...
        document.Info.Author = "Matheus Teixeira";
    ...
    private void CreateHeaderWithProfilePhotoAndName(Section page)
    {
        ...
        var pathFile = Path.Combine(directoryName!, "Logo", "ProfilePic.png");

        var image = row.Cells[0].AddImage(pathFile);
        image.Width = "62";

        row.Cells[1].AddParagraph("Hey, Matheus Teixeira");
